...
const BULLION_ENTITY_TYPES = ['inventory_logs', 'orders', 'purchases'];
...
async function loadEntityTrails(system, { startDate, endDate, entityTypes, locationId }) {
    ...
    entityTypes.map(async (entityType) => {
      try {
        ...
        return {
          entityType,
          rows: [],
          error: error?.message || `Failed to load ${entityType} audit trails.`,
        };
      }
    }),
  );
  ...
  const atLocation = lists
    ...
    .filter((row) => idsEqual(locationIdOf(row), locationId));
...
async function hydrateDetails(system, rows, { productIds } = {}) {
  ...
  candidates.sort((a, b) => {
    ...
    const order = { cash_logs: 0, cash_transactions: 1, payments: 2, inventory_logs: 3, orders: 4, purchases: 5 };
