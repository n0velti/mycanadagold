 ...
 * A System Admin / GM / HR can save the OAuth app and one shared HR connection
 * on the server. Regular staff then read employees through the proxy with no
 * personal Rippling token. A leftover personal token is still forwarded if present.
...
export const RIPPLING_DEVELOPER_URL = 'https://developer.rippling.com';
 ...
 */
export function canManageCompanyRippling(profile) {
...
export async function loadRipplingOAuthApp() {
  ...
  return {
    ...
    tokenSource: String(payload?.tokenSource || '').trim(),
  };
}

export async function saveRipplingOAuthApp({ clientId, clientSecret }) {
  const payload = await proxyJson('rippling/oauth/app', {
    method: 'POST',
...
export async function disconnectCompanyRippling() {
  await proxyJson('rippling/company/disconnect', { method: 'POST' });
}
...
async function ripplingFetch(pathWithQuery, { token } = {}) {
  ...
  if (token) options.upstreamAuthorization = `Bearer ${token}`;

  let response;
  try {
    response = await proxyFetch(`rippling${clean}`, options);
  } catch (error) {
    if (error instanceof ProxyError) throw error;
