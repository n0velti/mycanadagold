...
export function mapRingCentralAccount(row) {
  ...
  return {
    ...
    phoneNumbers: Array.isArray(row.phone_numbers || row.phoneNumbers)
      ? (row.phone_numbers || row.phoneNumbers)
          .map((item) => mapRingCentralNumber(typeof item === 'string' ? { phoneNumber: item } : item))
          .filter(Boolean)
      : [],
...
async function listFromSupabase() {
  ...
  let { data, error } = await supabase
    ...
    .order('store_name');
  if (error && /phone_numbers/i.test(String(error.message || ''))) {
    ({ data, error } = await supabase
      .from('ringcentral_accounts')
      .select(ACCOUNT_SELECT.replace(', phone_numbers', ''))
      .order('store_name'));
  }
...
export async function saveRingCentralAccount(values, userId) {
  ...
  const payload = await proxyJson('ringcentral/save', {
    ...
    body: JSON.stringify({
      ...
      jwt,
      serverUrl,
      phoneNumbers: Array.isArray(values?.phoneNumbers) ? values.phoneNumbers : undefined,
      updatedBy: userId || '',
    }),
  });
...
export function mapRingCentralNumber(row) {
  ...
  return {
    ...
    siteName: asString(row.siteName || row.site_name),
  };
}
