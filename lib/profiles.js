 ...
 * `aureus-login` Edge Function. Active staff can read teammate rows (name,
 * photo, store) so portraits, likes, and comments work for every role. The
 * client may update its own preference columns (pinned_tools, apps_view,
 * avatar_url) and team membership (team_id, is_team_intake).
 */
...
export const DEFAULT_APPS_VIEW = 'list';
...
const PROFILE_COLUMNS_WITHOUT_TEAM =
...
export async function fetchOwnProfile(supabase, userId) {
  ...
  let result = await query(PROFILE_COLUMNS);
  if (result.error && /team_id|is_team_intake/i.test(result.error.message || '')) {
    result = await query(PROFILE_COLUMNS_WITHOUT_TEAM);
  }
  if (result.error && /employee_type|avatar_url/i.test(result.error.message || '')) {
    result = await query(LEGACY_PROFILE_COLUMNS);
  }
  if (result.error) throw result.error;

  const row = result.data;
  if (row?.team_id) {
    const team = await supabase.from('teams').select('name').eq('id', row.team_id).maybeSingle();
    if (!team.error && team.data?.name) row.team_name = team.data.name;
  }
  return row;
}
...
/** Aureus "Employee" (not Manager / Admin) is locked to today on Home. */
export function isRestrictedHomeEmployee(profile) {
  return posEmployeeType(profile) === 'employee';
}

/**
 * PMA and branch managers are locked to their allocated store in apps.
 * Home still lists every store; Filter on Home controls dollar totals and
 * whether they can open another store's details.
 * General managers, admins, HR, and triage see every retail store in apps.
 */
export function isStoreScopedProfile(profile) {
  if (hasFullAppAccess(profile)) return false;
  const role = normalizeAppRole(profile?.appRole);
  return role !== 'general_manager' && role !== 'system_admin' && role !== 'hr' && role !== 'triage';
}
...
/** Assigned store for scoped roles; empty when the person may see every store. */
export function scopedStoreName(profile) {
  if (!isStoreScopedProfile(profile)) return '';
  return allocatedStoreName(profile);
}
