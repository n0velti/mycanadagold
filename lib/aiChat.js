...
function compactBullionAudit(audit, locationName) {
  const unbalanced = (audit.rows || [])
    .map((row) => {
      const counted =
        row.amount == null && row.vaultCount == null && row.nightCount == null
          ? null
          : roundQty((row.vaultCount || 0) + (row.nightCount || 0));
...
function trimContext(payload) {
  ...
  for (const key of ['transactions', 'financials', 'inventory', 'fintrac', 'audit', 'trends']) {
