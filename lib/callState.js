...
export function applyPresence(state, storeKey, liveCalls, now = Date.now()) {
  ...
  for (const [key, existing] of Object.entries(state.calls)) {
    ...
    if (!fresh) {
      // Softphone / in-browser calls stay until SIP or hang-up says they are
      // over. Presence often drops a party the moment we answer it (the leg
      // moves to a new INVITE) and would otherwise wipe the live card.
      if (!existing.sip && !existing.web) {
        delete calls[key];
        changed = true;
      }
      continue;
    ...
    next.presence = true;
    if (!existing.sip) {
      next = { ...next, status: fresh.status, direction: fresh.direction };
      if (isConnectedStatus(fresh.status) && !next.answeredAt) next.answeredAt = now;
