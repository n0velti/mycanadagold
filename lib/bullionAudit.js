...
/** Stores available in Audit → Bullion (retail cash set plus Workshop vault). */
export const AUDIT_BULLION_STORES = [...AUDIT_CASH_STORES, 'Workshop'];

function storeSortRank(name) {
  if (HOME_STORES.includes(name)) return 0;
  if (QUEBEC_STORES.includes(name)) return 1;
  if (namesMatch(name, 'Workshop')) return 2;
  return 3;
}
...
export async function fetchInventoryLogs(
  ...
  const parts = await Promise.all(
    chunks.map(async (chunk) => {
      ...
      const response = await posFetch(`${baseUrl}/inventory_logs?${params.toString()}`, {
        method: 'GET',
...
/** Physical count that should match system qty: vault + night + afternoon. */
export function countedPhysicalTotal(vaultCount, nightCount, afternoonCount) {
  return (
    (toNumber(vaultCount) ?? 0) +
    (toNumber(nightCount) ?? 0) +
    (toNumber(afternoonCount) ?? 0)
  );
}

/**
 * Aureus vault_count is vault+night+afternoon. Recover the vault remainder
 * using shift counts stored in Supabase.
 */
export function vaultRemainderFromAureus(aureusVault, nightCount, afternoonCount) {
  const aureus = toNumber(aureusVault);
  if (aureus == null) return null;
  return aureus - (toNumber(nightCount) ?? 0) - (toNumber(afternoonCount) ?? 0);
}
...
export async function saveInventoryLog(
  ...
  const response = await posFetch(`${baseUrl}/inventory_logs`, {
    method: 'POST',
    headers: authHeaders(token),
 ...
 */
export async function fetchBullionAudit(session, { date, locationId, systemKey, storeName }) {
  ...
  const [inventory, logs, shiftCounts] = await Promise.all([
  ...
  const rows = products
    .map((product) => {
      ...
      const nightCount = shiftCounts.night[id] ?? null;
      const afternoonCount = shiftCounts.afternoon[id] ?? null;
      const vaultEntered = shiftCounts.vaultEntered[id];
      const storeEntered = shiftCounts.storeEntered?.[id];
      const otherEntered = shiftCounts.otherEntered?.[id];
      const aureusVault = todayLog ? toNumber(todayLog.vault_count) : null;
      const vaultCount =
        aureusVault != null
          ? vaultRemainderFromAureus(aureusVault, nightCount, afternoonCount)
          : vaultEntered ?? null;
      const history = {};
      ...
      return {
        ...
        storeCount:
          storeEntered != null
            ? storeEntered
            : todayLog
              ? toNumber(todayLog.store_count)
              : null,
        otherCount:
          otherEntered != null
            ? otherEntered
            : todayLog
              ? toNumber(todayLog.other_count)
              : null,
