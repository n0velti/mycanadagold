...
export const MIN_PRICE_TOLERANCE = 0.001;
export const MAX_PRICE_TOLERANCE = 0.2;

/** POS sometimes stores the line total in the unit-price field. */
const UNIT_AS_TOTAL_TOLERANCE = 0.01;

export function normalizePriceTolerance(value, fallback = PRICE_TOLERANCE) {
  const n = Number(value);
  if (!Number.isFinite(n) || n <= 0) return fallback;
  return Math.min(MAX_PRICE_TOLERANCE, Math.max(MIN_PRICE_TOLERANCE, n));
}

export function formatPriceTolerance(tolerance = PRICE_TOLERANCE) {
  const pct = Math.round(normalizePriceTolerance(tolerance) * 1000) / 10;
  return `${String(pct).replace(/\.0$/, '')}%`;
}

export function catalogSnapshotKey(catalog) {
  const updated = String(catalog?.updated || '').trim();
  if (updated) return updated.slice(0, 120);
  const fetched = String(catalog?.fetchedAt || '').trim();
  if (fetched) return fetched.slice(0, 16);
...
export function snapshotPriceCatalog(catalog) {
  ...
  return {
    ...
    key: catalog.key || catalogSnapshotKey(catalog),
    entries,
  };
}
...
const BULLION_NAME_RE =
  /\b(gml|sml|maplegram|maple leaf|gold maple|silver maple|eagle|seagle|buffalo|krugerrand|pamp|britannia|panda|sovereign|philharmonic|nugget|perth|gbar|sbar|sround|bullion|credit\s*suisse)\b/i;
const BULLION_TYPE_RE = /^(bullion|coin|bar|numismatic)$/i;
...
function isNamedBullionProduct(line) {
  ...
  if (/\b[gs]\s*[-]?(?:round|bar|ml)s?\b/i.test(text)) return true;
...
function isBullionNamed(line) {
  ...
  if (BULLION_TYPE_RE.test(type)) return true;
  const group = String(line?.productGroup || '').toLowerCase();
  return group === 'bullion' || /\bbullion\b/.test(group);
}
...
export function isScrapJewelleryLine(line) {
  ...
  if (isBullionNamed(line)) return false;
  return looksLikeScrap(line);
}

/** Named bullion / coin / bar bought to resell in-store — not scrap for melt. */
export function isBullionResaleLine(line) {
  if (!line) return false;
  if (isBullionNamed(line)) return true;
  return false;
}

/**
 * Whether a purchase belongs on the melt triage list.
 * Bullion-only POs stay in store for resale and are not expected at the Workshop.
 * Mixed POs still come in (scrap/jewellery) but `hasHeldBullion` flags the rest.
 */
export function classifyPurchaseForTriage(row) {
  ...
  const consider = lines.length ? lines : [fallback];
  let scrap = 0;
  let bullion = 0;
  let other = 0;
  for (const line of consider) {
    if (isBullionResaleLine(line)) bullion += 1;
    else if (isScrapJewelleryLine(line)) scrap += 1;
    else other += 1;
  }
  if (!lines.length && isBullionResaleLine(fallback) && !isScrapJewelleryLine(fallback)) {
    bullion = Math.max(bullion, 1);
    other = 0;
  }
  const bullionOnly = bullion > 0 && scrap === 0 && other === 0;
  return {
    expectedAtTriage: !bullionOnly,
    bullionOnly,
    hasHeldBullion: bullion > 0,
    bullionCount: bullion,
    scrapCount: scrap,
  };
}

export function stampTriagePurchase(row) {
  if (!row) return row;
  const cls = classifyPurchaseForTriage(row);
  return {
    ...row,
    hasHeldBullion: cls.hasHeldBullion,
    bullionOnly: cls.bullionOnly,
    bullionLineCount: cls.bullionCount,
    scrapLineCount: cls.scrapCount,
  };
}
...
function enteredScrapTotal(line, weightGrams, rate, tolerance) {
  const lineTotal = Number(line?.lineTotal);
  if (Number.isFinite(lineTotal) && lineTotal > 0) return lineTotal;
  const unitPrice = Number(line?.unitPrice);
  if (!Number.isFinite(unitPrice) || unitPrice <= 0) return null;
  if (withinPriceTolerance(unitPrice, rate, tolerance) || unitPrice < rate * 4) return unitPrice * weightGrams;
  if (withinPriceTolerance(unitPrice, rate * weightGrams, tolerance)) return unitPrice;
  const qty = Number(line?.quantity);
...
function checkSilverGramRates(line, catalogEntries, meta, tolerance) {
  ...
  const hit = silverGramEntries(catalogEntries).find((entry) => {
    const unitHit = actualUnit != null && withinPriceTolerance(actualUnit, entry.buyAmount, tolerance);
    const totalHit =
      actualTotal != null &&
      weightGrams &&
      withinPriceTolerance(actualTotal, entry.buyAmount * weightGrams, tolerance);
...
function checkScrapLine(line, catalogEntries, txType, tolerance) {
  ...
  if (meta.metal === 'silver' && !meta.silverKind) {
    return checkSilverGramRates(line, catalogEntries, meta, tolerance);
  }
  ...
  const actualTotal = enteredScrapTotal(line, meta.weightGrams, rate, tolerance);
  const tierLabel = jewelleryTierLabel(meta, matched);
  const rateLabel = matched.buyLabel || `$${rate.toFixed(2)}/g`;
  const unitOk = hasUnit && withinPriceTolerance(unitPrice, rate, tolerance);
  const totalOk = actualTotal != null && withinPriceTolerance(actualTotal, expectedTotal, tolerance);
  const base = {
    name,
  ...
  if (unitOk || totalOk) {
    return scrapResult({
      ...base,
      status: 'ok',
      reason: `${tierLabel} at ${rateLabel} × ${formatGrams(meta.weightGrams)} = ${formatMoney(expectedTotal)}, within ${formatPriceTolerance(tolerance)}.`,
    });
  }
  ...
  const wrong = others.find((entry) => {
    ...
    return (
      (hasUnit && withinPriceTolerance(unitPrice, entry.buyAmount, tolerance)) ||
      (actualTotal != null && withinPriceTolerance(actualTotal, otherTotal, tolerance))
    );
  });
...
export function checkPricedLine(line, catalogEntries, txType, tolerance = PRICE_TOLERANCE) {
  if (isScrapJewelleryLine(line) || (looksLikeScrap(line) && !isNamedBullionProduct(line))) {
    return checkScrapLine(line, catalogEntries, txType, tolerance);
  }
  ...
  const actual =
    ...
    lineTotal > 0 &&
    Math.abs(listedUnit - lineTotal) / lineTotal <= UNIT_AS_TOTAL_TOLERANCE
      ? lineTotal / quantity
      : listedUnit;
  ...
  const hitCorrect = correct.find((amount) => withinPriceTolerance(actual, amount, tolerance));
  if (hitCorrect != null) {
    const row =
      candidates.find((entry) =>
        withinPriceTolerance(actual, isPurchase ? entry.buyAmount : entry.sellAmount, tolerance),
      ) || candidates[0];
    return {
      status: 'ok',
      name,
      ...
      reason: isPurchase
        ? `Website we-buy is ${row.buyLabel}, within ${formatPriceTolerance(tolerance)}.`
        : `Website we-sell is ${row.sellLabel}, within ${formatPriceTolerance(tolerance)}.`,
    };
  }

  const hitWrong = opposite.find((amount) => withinPriceTolerance(actual, amount, tolerance));
  const hitOtherSell = !isPurchase
    ? null
    : allSell.find((amount) => withinPriceTolerance(actual, amount, tolerance));
...
export function checkTransactionPrices(row, catalog, { tolerance = PRICE_TOLERANCE } = {}) {
  const tol = normalizePriceTolerance(tolerance);
  const lines = Array.isArray(row?.pricedLines) ? row.pricedLines : [];
  const entries = catalogEntries(catalog);
  const isPurchase = row?.type === 'purchase';
  const results = lines.map((line) => checkPricedLine(line, entries, row?.type, tol));
  ...
  const meta = {
    ...
    frozen: Boolean(catalog?.entries),
  };
  ...
  if (
    !catalog &&
    !checked.length &&
    lines.some((line) => Number(line?.unitPrice) > 0 || Number(line?.lineTotal) > 0)
  ) {
    return { status: 'loading', ...meta };
  }
  if (!checked.length) {
