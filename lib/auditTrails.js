...
export async function fetchAuditTrailList(
  ...
  while (page <= MAX_PAGES) {
    ...
    const response = await posFetch(`${baseUrl}/audit_trails?${params.toString()}`, {
      method: 'GET',
...
export async function fetchAuditTrailDetail(token, id, baseUrl = API_BASE_URL) {
  const response = await posFetch(`${baseUrl}/audit_trails/${id}`, {
    method: 'GET',
    headers: authHeaders(token),
  });
  const payload = await parseJsonResponse(response);
 ...
 * When the count is off, also loads sale, purchase, and transfer changes.
 * Dates on the document vs when the change was made, plus previous→new amounts.
 */
export async function gatherCashAuditTrails(
  session,
  { date, previousDate, locationId, systemKey, onProgress, includeDocuments = false } = {},
) {
  ...
  const entityTypes = includeDocuments
    ? [...CASH_ENTITY_TYPES, 'orders', 'purchases', 'transfers']
    : CASH_ENTITY_TYPES;
  onProgress?.(
    includeDocuments
      ? 'Loading cash, sale, purchase, and transfer logs…'
      : 'Loading POS audit trails…',
  );
  let loaded;
  try {
  ...
  const documentTrails = trails.filter(
    (trail) =>
      trail.entityType === 'orders' ||
      trail.entityType === 'purchases' ||
      trail.entityType === 'transfers',
  );
  ...
  return {
    ...
    documentEvents: documentTrails.length ? payloadEvents(documentTrails) : [],
