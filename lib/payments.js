...
import { isDebitMethod } from './paymentMethod';
...
async function fetchPaymentsForSystem(
  ...
  const response = await posFetch(`${system.baseUrl}/payments?${params}`, {
    method: 'GET',
...
/**
 * Cleared debit / Interac / EFT payments for a calendar day.
 * Aureus category names vary, so this loads all methods and classifies locally.
 */
export async function fetchDebitPayments(
  session,
  { date, locationId, tillId, storeName, systemKey } = {},
) {
  const result = await fetchDayPayments(session, {
  ...
  const rows = result.rows.filter((row) => isDebitMethod(row.paymentType));
  return {
    ...result,
    rows,
    storeSummaries: summarizeCashByStore(rows),
    totals: summarizeCashTotals(rows),
  };
}
