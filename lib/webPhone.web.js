...
/**
 * The SDK rejects a failed WebSocket with the raw error Event, which has no
 * `message`. Without this, the UI can only say "could not register".
 */
function sipSocketError(err, sipInfo, { backup = false } = {}) {
  if (err instanceof SipTimeoutError) return err;
  if (err instanceof Error && err.message && err.code !== 'sip_socket') return err;
  const host = String((backup ? sipInfo?.outboundProxyBackup : sipInfo?.outboundProxy) || '')
    .replace(/^wss?:\/\//i, '')
    .trim();
  const wrapped = new Error(
    host
      ? `Could not reach RingCentral’s phone server (${host}).`
      : 'Could not reach RingCentral’s phone server.',
  );
  wrapped.name = 'SipSocketError';
  wrapped.code = 'sip_socket';
  wrapped.cause = err;
  return wrapped;
}
...
/**
 * Safari iOS only grants getUserMedia() reliably inside the Answer/Call tap;
 * a request made later, after the SIP round trips, is refused with
 * "SecurityError: The operation is insecure." So capture happens once, in the
 * tap, and every call session sends a clone of that one stream. The SDK never
 * touches getUserMedia: `webRtcSessionFactory` below hands it our own WebRTC
 * session, so the only permission-sensitive call left is ours.
 */
let primedMic = null;
...
/**
 * Every microphone failure leaves this module as an Error with
 * `code: 'microphone'` and the browser's own error name in `reason`, so the
 * UI can tell a refused microphone from a SIP or WebRTC failure instead of
...
function microphoneFailure(err, fallback = 'This browser has no microphone access.') {
  if (isMicrophoneFailure(err)) return err;
  const reason = String(err?.name || '');
  const detail = String(err?.message || '').trim();
  const wrapped = new Error(reason && detail ? `${reason}: ${detail}` : detail || reason || fallback);
  wrapped.name = 'MicrophoneError';
  wrapped.code = 'microphone';
  wrapped.reason = reason;
  wrapped.cause = err;
  return wrapped;
}

/** A WebRTC failure in our own session (not the microphone, not SIP). */
function webRtcFailure(err, step) {
  if (err?.code === 'microphone' || err?.code === 'webrtc') return err;
  const reason = String(err?.name || '');
  const detail = String(err?.message || '').trim();
  const wrapped = new Error(`${reason && detail ? `${reason}: ${detail}` : detail || reason || 'WebRTC failed'} (${step})`);
  wrapped.name = 'WebRtcError';
  wrapped.code = 'webrtc';
  wrapped.reason = reason;
  wrapped.step = step;
  wrapped.cause = err;
...
function requestMicrophone(constraints) {
  ...
  if (!media?.getUserMedia) {
    return Promise.reject(microphoneFailure(null));
  }
  return Promise.resolve(media.getUserMedia(constraints)).catch((err) => {
    throw microphoneFailure(err);
  });
}
...
function startMicPrime() {
  ...
  primedMicPromise = requestMicrophone({ audio: true, video: false })
    .then((stream) => {
      primedMic = stream;
      primedMicPromise = null;
...
/**
 * Ask for the microphone up front so a refused permission surfaces as a clear
 * error before any SIP signalling starts. The stream is kept for the length
 * of the call; sessions send clones of it and never call getUserMedia.
 */
export async function ensureMicrophone() {
  const stream = await startMicPrime();
  if (!liveAudioTracks(stream).length) {
    primedMic = null;
    throw microphoneFailure(null, 'The microphone stopped before the call could start.');
  }
  return stream;
}

/** A fresh track for one call, cloned from the primed capture. */
async function cloneMicrophoneTrack() {
  const stream = await ensureMicrophone();
  return liveAudioTracks(stream)[0].clone();
}

// ---------------------------------------------------------------------------
// WebRTC session
//
// ringcentral-web-phone lets the application own WebRTC through
// `webRtcSessionFactory` (README, "delegated WebRTC"). The SDK keeps SIP
// signalling, call state, re-INVITEs and hold SDP; we keep the peer
// connection, so the microphone comes from the stream opened in the tap and
// the SDK's own getUserMedia/enumerateDevices path (which Safari iOS refuses
// after an await) is never reached.
// ---------------------------------------------------------------------------

const ICE_GATHER_TIMEOUT_MS = 2_000;

function iceServersFor(stunServers) {
  return (stunServers || [])
    .map((url) => String(url || '').trim())
    .filter(Boolean)
    .map((url) => ({ urls: /^(stun|turn)s?:/i.test(url) ? url : `stun:${url}` }));
}
...
class BrowserRtcSession {
  constructor({ stunServers } = {}) {
    this.pc = new RTCPeerConnection({ iceServers: iceServersFor(stunServers) });
    this.sender = null;
    this.muted = false;
    this.disposed = false;
  }

  /** Put a live microphone track on the connection (once, or again if it ended). */
  async ensureLocalTrack() {
    if (this.disposed) throw webRtcFailure(new Error('Session closed'), 'send audio');
    if (this.sender?.track && this.sender.track.readyState === 'live') return;
    const track = await cloneMicrophoneTrack();
    track.enabled = !this.muted;
    try {
      if (this.sender) await this.sender.replaceTrack(track);
      else this.sender = this.pc.addTrack(track);
    } catch (err) {
      track.stop();
      throw webRtcFailure(err, 'send audio');
    }
    try {
      const params = this.sender.getParameters();
      if (params.encodings?.length) {
        for (const encoding of params.encodings) encoding.priority = 'high';
        this.sender.setParameters(params).catch(() => {});
      }
    } catch {
      // Priority is a hint; not every browser accepts it.
    }
  }

  async createOffer({ iceRestart = false } = {}) {
    await this.ensureLocalTrack();
    try {
      const offer = await this.pc.createOffer({ iceRestart: Boolean(iceRestart) });
      await this.pc.setLocalDescription(offer);
      await waitForIceGathering(this.pc);
      return this.pc.localDescription.sdp;
    } catch (err) {
      throw webRtcFailure(err, 'create offer');
    }
  }
  ...
  async createAnswer(offerSdp) {
    ...
    try {
      ...
      throw webRtcFailure(err, 'create answer');
    }
  }
  ...
  async applyAnswer(answerSdp) {
    try {
      ...
      throw webRtcFailure(err, 'apply answer');
    }
  }

  async changeInputDevice(deviceId) {
    const stream = await requestMicrophone({ video: false, audio: deviceId ? { deviceId: { exact: deviceId } } : true });
    const track = stream.getAudioTracks()[0];
    if (!track) throw microphoneFailure(null, 'That microphone has no audio.');
    track.enabled = !this.muted;
  ...
  async changeOutputDevice() {
    // The far end plays through our own <audio> element (see attachRemoteAudio).
  }

  setMuted(muted) {
    this.muted = Boolean(muted);
    for (const sender of this.pc.getSenders()) {
      if (sender.track) sender.track.enabled = !this.muted;
    }
  }

  sendDtmf(tones, duration, interToneGap) {
    for (const sender of this.pc.getSenders()) {
      if (sender.dtmf?.canInsertDTMF) sender.dtmf.insertDTMF(tones, duration, interToneGap);
    }
  }
...
function createWebRtcSession(context) {
  return new BrowserRtcSession(context);
}

/** The peer connection behind an SDK session, whichever mode created it. */
function peerConnectionOf(session) {
  return session?.webRtcSession?.pc || session?.rtcPeerConnection || null;
}
...
export function primeCallAudio() {
  // Start capture inside the click/tap: Safari iOS refuses the mic after an
  // await. Errors surface from ensureMicrophone() / answer(), not here.
  startMicPrime().catch(() => {});
...
export function toCallSnapshot(session, storeKey, storeName, extra = {}) {
  ...
  // Call Control (Answer with deviceId, call-out) delivers its leg as an INVITE
  // carrying "Alert-Info: Auto Answer"; a person dialling the store never does.
  const autoAnswer = /auto[\s-]?answer/i.test(safe(() => session.sipMessage?.getHeader?.('Alert-Info')));
  return {
    id,
    storeKey,
    storeName,
 ...
 *   onAnswerError(snapshot, err)         the SDK's own auto-answer of an INVITE failed
 */
export async function startWebPhone({
  ...
  // The SIP/WebRTC SDK is sizeable and only needed once a store phone is
  // actually registered, so it ships as its own chunk instead of in the
  ...
  // webRtcSessionFactory: media is ours (see BrowserRtcSession), so the SDK
  // never calls getUserMedia.
  const phone = new WebPhone({
    sipInfo,
    instanceId: webPhoneInstanceId(extensionId),
    autoAnswer: true,
    webRtcSessionFactory: createWebRtcSession,
    debug,
  });
  ...
  /** Sessions the app is answering through handle.answer(); their errors return to the caller. */
  const manualAnswers = new WeakSet();
  let disposed = false;
  ...
  const track = (session, first) => {
    ...
    // The SDK answers an "Alert-Info: Auto Answer" INVITE by itself from a
    // timer and drops the rejection. Report it, unless the app asked (then
    // handle.answer() below already returns the error to the caller).
    if (session.direction === 'inbound' && typeof session.answer === 'function') {
      const sdkAnswer = session.answer.bind(session);
      session.answer = () =>
        sdkAnswer().catch((err) => {
          if (!manualAnswers.has(session)) onAnswerError?.({ ...snapshot }, err);
          throw err;
        });
    }
    ...
    connecting = (async () => {
      ...
      try {
        ...
        try {
          ...
          // The primary SIP proxy failed to open. RingCentral hands out a
          // backup in sipInfo; the SDK only dials it when told to.
          const backup = String(sipInfo?.outboundProxyBackup || '').trim();
          const primary = String(sipInfo?.outboundProxy || '').trim();
          const client = phone.sipClient;
          if (disposed || !backup || backup === primary || client?.useBackupOutboundProxy) {
            throw sipSocketError(err, sipInfo);
          }
          client.toggleBackupOutboundProxy?.(true);
          ...
          try {
            ...
            client.toggleBackupOutboundProxy?.(false);
            throw sipSocketError(backupErr, sipInfo, { backup: true });
          }
        }
        if (disposed) return;
        reconnectAttempt = 0;
