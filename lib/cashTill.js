...
/**
 * Aureus location cash_logs expose two drawers on one record:
 * start_balance / physical = CAD, start_balance_usd / physical_usd = USD.
 * CAD start_balance is never used as USD. A USD till's start_balance is used
 * only when the payload has no *_usd fields and that till's currency is USD.
 */
function parseCashLogs(payload, dateKey) {
  ...
  const locationRow =
    ...
    rows.find((row) => rowHasMoneyKeys(row, ['start_balance_usd', 'physical_usd'])) ||
    rows.find((row) => !row.till_id && !row.till?.id) ||
    rows[0] ||
    {};
  const tillRows = rows.filter(
    (row) => row !== locationRow && (row?.till_id || row?.till?.id || logTillName(row)),
  );
  const cadTill1Row =
    tillRows.find((row) => isCadTill1Log(row) && /till\s*1\b/i.test(logTillName(row))) || null;
  const cadTillRow = cadTill1Row || tillRows.find(isCadTill1Log) || null;
  const usdTillRow = tillRows.find(isUsdCashLog) || null;
  const usdFieldRows = [locationRow, cadTill1Row, cadTillRow, usdTillRow, ...rows];
  const hasUsdFields = usdFieldRows.some((row) =>
    rowHasMoneyKeys(row, ['start_balance_usd', 'physical_usd']),
  );

  const cadStart =
    readMoney(cadTill1Row, ['start_balance', 'startBalance', 'opening_balance', 'opening']) ??
    readMoney(locationRow, ['start_balance', 'startBalance']) ??
    readMoney(cadTillRow, ['start_balance', 'startBalance', 'opening_balance', 'opening']) ??
    0;
  const cadPhysical =
    readMoney(cadTill1Row, ['physical', 'physical_count']) ??
    readMoney(locationRow, ['physical', 'physical_count']) ??
    readMoney(cadTillRow, ['physical', 'physical_count']) ??
    0;

  const usdStart =
    firstMoney(usdFieldRows, ['start_balance_usd']) ??
    (!hasUsdFields &&
    usdTillRow &&
    usdTillRow !== cadTillRow &&
    logCurrency(usdTillRow) === 'USD'
      ? readMoney(usdTillRow, ['start_balance', 'startBalance', 'opening_balance', 'opening'])
      : null) ??
    0;
  const usdPhysical =
    firstMoney(usdFieldRows, ['physical_usd']) ??
    (!hasUsdFields &&
    usdTillRow &&
    usdTillRow !== cadTillRow &&
    logCurrency(usdTillRow) === 'USD'
      ? readMoney(usdTillRow, ['physical', 'physical_count'])
      : null) ??
    0;

  return {
    date: dateKey,
    startBalance: cadStart,
    startBalanceUsd: usdStart,
    physical: cadPhysical,
    physicalUsd: usdPhysical,
    hasUsdBalanceFields: hasUsdFields,
    raw: payload,
  };
}
...
export async function fetchStoreCashPosition(session, { storeName, date } = {}) {
  ...
  return {
    ...
    yesterday: {
      ...
      physical: yesterdayLog.physical,
      physicalUsd: yesterdayLog.physicalUsd,
    },
    todayLog,
