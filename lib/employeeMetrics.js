import { findStaffByEmployeeName, getCategory, normalizeAppRole, staffDisplayName } from './permissions';
...
export const MEASURES = [
  ...
  },
  {
    ...
    label: 'Hours',
    format: 'hours',
    hint: 'Time from the Rippling time CSV in this date range. A person’s home store is used, because that file has no store',
  },
];
...
function addFact(bucket, fact) {
  ...
  if (fact.kind === 'hours') {
    bucket.minutes += Number(fact.minutes) || 0;
    return;
  }
  if (fact.kind === 'error') {
    bucket.errors += 1;
...
export function pivotFacts(facts, { groupBy = 'store', thenBy = '', measure = 'staffDays' } = {}) {
  ...
  for (const fact of facts) {
    if (fact.kind === 'hours' && (rowDim === 'txType' || colDim === 'txType')) continue;
...
function employeeKeyForPerson(person, sales, fallback) {
  if (person?.id) {
    const counts = new Map();
    for (const fact of sales || []) {
      if (fact.employeeId !== person.id || !fact.employee || fact.employee === '—') continue;
      counts.set(fact.employee, (counts.get(fact.employee) || 0) + 1);
    }
    const best = [...counts.entries()].sort((a, b) => b[1] - a[1])[0];
    if (best) return best[0];
  }
  return staffDisplayName(person) || fallback;
}

/** Hours from the Rippling time CSV, lined up with the same employee names as sales. */
export function factsFromTimeEntries(rows, staff = [], { storeFilter, sales = [] } = {}) {
  const lock = String(storeFilter || '').trim();
  const facts = [];
  for (const row of Array.isArray(rows) ? rows : []) {
    const csvName = String(row?.name || row?.employee_name || '').trim();
    const date = String(row?.date || row?.entry_date || '').slice(0, 10);
    const minutes = Number(row?.minutes) || 0;
    if (!csvName || !/^\d{4}-\d{2}-\d{2}$/.test(date) || minutes <= 0) continue;
    const person = findStaffByEmployeeName(staff, csvName);
    const homeStore = String(person?.locationName || '').trim();
    if (lock && (!homeStore || homeStore.localeCompare(lock, undefined, { sensitivity: 'base' }) !== 0)) {
      continue;
    }
    facts.push({
      kind: 'hours',
      employee: employeeKeyForPerson(person, sales, csvName),
      employeeId: person?.id || '',
      store: homeStore || '—',
      homeStore,
      role: jobRoleLabel(person),
