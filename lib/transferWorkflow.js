...
export function addStandaloneTriagePo(row) {
  ...
  const leftover = state.deleted.filter((entry) => {
    if (entry.kind === 'doc') return entry.sourceId !== stampedId;
    if (entry.kind === 'po') return flattenBatchPos(entry.payload).every((item) => item.id !== stampedId);
    return true;
  });
  if (leftover.length !== state.deleted.length) {
    for (const entry of state.deleted) {
      if (!leftover.some((row) => row.id === entry.id)) removedDeletedIds.add(entry.id);
    }
    commit({ deleted: leftover });
  }
...
export function saveTriagePoReview(poId, review) {
  ...
  commit({
    ...
    triage: state.triage.map((row) => applyPoReviewToTransfer(row, poId, review)),
  });
