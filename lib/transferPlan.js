...
/** Thousandths, matching POS transfer quantities (3.5 stays 3.5). */
...
/**
 * Largest-remainder allocation so targets sum to `total`.
 * Whole totals stay whole units. Fractional totals (3.5) keep thousandths.
 */
export function allocateByPercent(total, percentByKey) {
  ...
  if (total <= 0 || weightSum <= 0 || entries.length === 0) return result;

  const scale = isWholeQty(total) ? 1 : 1000;
  const totalUnits = Math.round(total * scale);
  const raw = entries.map(([key, pct]) => {
    const exact = (totalUnits * toNumber(pct)) / weightSum;
  ...
  for (const row of raw) result[row.key] = row.floor / scale;
  return result;
}
...
export function buildForwardMoves(orderedStoreIds, currentById, targetById, storeById) {
  ...
  for (let i = 0; i < nodes.length; i += 1) {
    ...
    for (let j = i + 1; j < nodes.length && nodes[i].avail > 0; j += 1) {
      ...
      const qty = roundQty(Math.min(nodes[i].avail, nodes[j].need));
      if (qty <= 0) continue;
      moves.push({
        fromId: nodes[i].id,
        toId: nodes[j].id,
      ...
      nodes[i].avail = roundQty(nodes[i].avail - qty);
      nodes[j].need = roundQty(nodes[j].need - qty);
    }
  }

  const unmetNeed = roundQty(nodes.reduce((sum, node) => sum + node.need, 0));
  const stranded = roundQty(nodes.reduce((sum, node) => sum + node.avail, 0));
...
function buildStopSheets(orderedStores, movingProducts, catalogProducts) {
  return orderedStores.map((store, index) => {
      ...
      for (const move of product.moves) {
        if (move.fromId === store.id) {
          outs.push({
            ...
            partnerQty: Math.max(0, toNumber(product.currentById?.[move.toId])),
            partnerId: move.toId,
            partnerName: move.toName,
          });
          outByProduct.set(
            product.id,
            roundQty((outByProduct.get(product.id) || 0) + move.qty),
          );
        }
    ...
    for (const product of catalogProducts) {
      ...
      const stayQty = roundQty(current - shipped);
    ...
    return {
      ...
      ins,
      staying,
      outUnits: roundQty(outs.reduce((sum, row) => sum + row.qty, 0)),
      inUnits: roundQty(ins.reduce((sum, row) => sum + row.qty, 0)),
      stayingUnits: roundQty(staying.reduce((sum, row) => sum + row.stayQty, 0)),
...
export function applyMoveQtyOverrides(plan, overrides = {}) {
  ...
  for (const [, group] of groups.entries()) {
    ...
    const onHand = roundQty(Math.max(0, toNumber(product?.currentById?.[group[0].fromId])));
    let used = 0;
    for (const leg of group) {
      const next = roundQty(Math.min(leg.qty, Math.max(0, onHand - used)));
      leg.qty = next;
      used = roundQty(used + next);
    }
  }
...
export function computeTransferPlan({
  ...
  for (const row of rows) {
    ...
    for (const store of routeStores) {
      const qty = roundQty(Math.max(0, toNumber(row.quantities?.[store.id])));
      currentById[store.id] = qty;
      total = roundQty(total + qty);
    }
    ...
    .map((sheet) => {
      const outRows = sheet.outs
        ...
        <tr>
          ...
          <td class="num">${formatQty(row.partnerQty ?? 0)}</td>
        </tr>`,
        )
        .join('');
        ...
        ${
          sheet.outs.length
            ? `<h3>Transfer OUT (${formatQty(sheet.outUnits)})</h3>
               <table>
                 <thead><tr><th>Product</th><th>On hand</th><th>Send</th><th>To</th><th>To has</th></tr></thead>
                 <tbody>${outRows}</tbody>
               </table>`
