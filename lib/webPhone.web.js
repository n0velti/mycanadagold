...
/**
 * The SDK opens its capture stream with `deviceId: { exact: id }` from its
 * device manager. Its default takes the first `audioinput` row blindly, which
 * throws when the list is empty and fails with OverconstrainedError when the
 * id is blank (mobile Safari and Chrome hide ids until permission settles).
 * Returning `undefined` drops the constraint, so the browser's default
 * microphone is used instead of failing the answer.
 */
