...
export function buildReview(row, draft, { note = '', errorType = '', errorAmount = '', images = [], lineEdits = [] } = {}) {
  return {
    draft,
    ...
    lineEdits: (Array.isArray(lineEdits) ? lineEdits : []).map((line) => ({
      index: Number(line?.index),
      name: String(line?.name || '').trim(),
      originalAmount: String(line?.originalAmount || '').trim(),
      amount: formatErrorAmount(line?.amount),
    })),
  };
}
