...
export async function updateStaffAccess(userId, { appRole, isSystemAdmin, isActive }) {
  ...
  const supabase = getSupabase();
  const columns = [STAFF_COLUMNS, STAFF_COLUMNS_WITHOUT_TEAM, LEGACY_STAFF_COLUMNS];
  let lastError = null;
  for (const selectCols of columns) {
    const { data, error } = await supabase
      .from('profiles')
      .update(patch)
      .eq('id', id)
    ...
    if (error) {
      lastError = error;
      if (/team_id|is_team_intake|employee_type|avatar_url/i.test(error.message || '')) continue;
      throw error;
    }
    if (!data) throw new Error('That person could not be updated.');
    const mapped = mapStaffRow(data);
    if (mapped.appRole !== role) {
      throw new Error('The role was not saved. Only a System Admin can change roles.');
    }
    if (typeof isActive === 'boolean' && mapped.isActive !== isActive) {
      throw new Error('Access was not updated.');
    }
    return mapped;
  }
  throw lastError || new Error('That person could not be updated.');
}
