...
export async function receivePosTransfer(token, transferId, lines, baseUrl = API_BASE_URL) {
  ...
  const response = await fetch(`${baseUrl}/transfers/${encodeURIComponent(id)}/receive`, {
    method: 'POST',
    headers: authHeaders(token),
    body: JSON.stringify(body),
  });
...
export async function receivePlannedPosItems(
  ...
  for (const item of targets) {
    ...
    if (qty <= 0) {
      localUpdates[item.id] = { receivedQty: already, received: already >= item.sentQty };
      continue;
    }
  ...
  for (const group of groups.values()) {
    const toLocationId = group.transfer.to?.id;
    const toName = group.transfer.to?.name || '';
    if (!toLocationId) {
      throw new Error('POS did not return the receiving store.');
    }
    const { mapped, changed } = await ensureEmployeeAtLocation(token, employeeId, {
    ...
    if (changed) {
      await onLocationEnsured?.({
        locationId: mapped?.locationId || String(toLocationId),
        locationName: mapped?.locationName || toName,
      });
    }
