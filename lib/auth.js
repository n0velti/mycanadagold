...
function primaryAureusHost() {
  try {
    ...
    return 'canadagoldeast.aureuspos.com';
  }
}
...
export async function restoreSession({ onProfileSynced } = {}) {
  ...
  if (aureusCheck.ok) {
    ...
    const generation = aureusSessionGeneration;
    syncProfileFromAureus(supabase, stored.token, supabaseUser.id, profileRow)
      .then((nextRow) => {
...
async function syncProfileFromAureus(supabase, aureusToken, supabaseUserId, profileRow) {
  const synced = await invokeEdgeFunction('aureus-login', { action: 'sync-staff', aureusToken });
