 ...
 * restyles it with a locked 3D cartoon look so every profile belongs in the
 * same world while keeping that person's face.
 */
...
/**
 * Restyle a webcam still or library photo into the shared Canada Gold cartoon.
...
export async function stylizeAvatarPhoto(asset, { signal } = {}) {
  ...
  const response = await proxyFetch('avatars/stylize', {
    ...
    signal,
    upstreamApiKey: await getProviderApiKey('openai'),
  });
