/**
 * Rippling hours without the paid API. Scheduled reports are emailed as CSV to
 * one company inbox. That person connects it once; the server keeps the
 * permission and imports CSV attachments even when they are signed out.
 * Other people see the imported hours, not that mailbox.
 */
...
export const HOURS_MAILBOX = 'edward.anuichi@canadagold.ca';
/** How often the open app asks Gmail whether a newer Rippling CSV arrived. */
export const RIPPLING_MAIL_CHECK_MS = 7 * 60_000;
...
export function buildHoursAuthorizeUrl({ clientId, loginHint, hostedDomain }) {
  ...
  // Same Workspace Google app as Emails. The dedicated hours app is unverified,
  // so Google only allows its test users through.
...
function mapStatus(payload) {
  const reported = asString(payload?.email).toLowerCase();
  const linked = Boolean(payload?.connected) && reported === HOURS_MAILBOX;
  return {
    ...
    lastError: linked ? asString(payload?.lastError) : '',
    ...
    feedSource: payload?.feedSource === 'drop' ? 'drop' : 'email',
    ...
    syncError: linked ? asString(payload?.syncError) : '',
  };
}
...
export async function setHoursFeedSource(source) {
  return mapStatus(
    await proxyJson('rippling/time/source', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ source: source === 'drop' ? 'drop' : 'email' }),
    }),
  );
}

/**
 * Ask the proxy to look for a newer report. The open app calls this every 7
 * minutes. The server skips a second look inside that window, and the screens
 * keep using the newest CSV already imported. `force` bypasses the cooldown
 * and is honoured only for HR / GM / System Admin.
 */
...
export async function fetchRipplingCsvFiles() {
  const { data, error } = await getSupabase()
    ...
    .select('id, message_id, attachment_name, subject, source, received_at, row_count, kind, is_current')
