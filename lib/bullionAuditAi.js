...
function lineItemFields(item) {
  ...
  return {
    ...
    deliveredFlag,
    unitPrice: money.displayUnitPrice,
...
export function buildUnbalancedBullionItems(rows, drafts) {
  ...
  for (const row of rows) {
    ...
    const hasEntry =
      ...
      (draft.afternoon != null && String(draft.afternoon).trim() !== '') ||
...
export async function gatherBullionAuditContext(
  ...
  const enrichedUnbalanced = targets.map((item) => {
    ...
    const yAfternoon = toNumber(item.afternoonCount) ?? 0;
    const yCounted =
      yLog != null
        ? (toNumber(yLog.vault_count) ?? 0) + yNight + yAfternoon
...
function buildBullionAuditMessages(context) {
  const payload = {
    ...
    notes: {
      diffMeaning: 'diff = countedTotal (vault+night+afternoon) − systemCount. Negative = short vs system.',
      deliveryCheck:
...
Investigate why physical vault/night/afternoon counts do not match Aureus system inventory for specific bullion products at one store/day.
...
5. Look for wrong product/SKU, wrong qty, void/unpaid txs still affecting stock, transfers, or count entry mistakes (vault vs night vs afternoon vs store vs other).
