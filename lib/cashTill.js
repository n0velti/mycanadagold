...
function storeSystemRank(systemKey, storeName) {
  const quebec = QUEBEC_STORES.some((name) => namesMatch(name, storeName));
  if (quebec) {
...
function hasCashBalanceFields(row) {
...
function cashLogRows(payload) {
  ...
  const rows = [];
  if (!Array.isArray(payload) && hasCashBalanceFields(payload)) {
    rows.push(payload);
  }
  const nested = Array.isArray(payload)
    ? payload
    : Array.isArray(payload.data)
      ...
      : Array.isArray(payload.cash_logs)
        ...
        : payload.data && typeof payload.data === 'object'
          ? [payload.data]
          : [];
  for (const row of asObjectList(nested)) {
    if (row === payload) continue;
    rows.push(row);
    for (const child of nestedTillRows(row)) {
      if (child !== payload && child !== row) rows.push(child);
    }
  }
  return rows;
}
...
function logTillName(row) {
  return String(
    row?.till?.name || row?.till_name || (row?.till_id || row?.till?.id ? row?.name : '') || '',
  ).trim();
}

function rowHasMoneyKeys(row, keys) {
  if (!row || typeof row !== 'object') return false;
  return keys.some((key) => row[key] != null && row[key] !== '');
}

function firstMoney(rows, keys) {
  for (const row of rows) {
    const amount = readMoney(row, keys);
...
function rowTillId(row) {
  const id = row?.till_id ?? row?.till?.id;
  return id == null || id === '' ? '' : String(id);
}

/**
 * Till 1 CAD lives on a till-scoped cash_log. Location start_balance can be
 * CAD + USD converted into CAD — never use that mixed figure for Till 1.
 */
function pickCadBalanceRow(rows, tillId) {
  const list = Array.isArray(rows) ? rows.filter(Boolean) : [];
  const wanted = tillId == null || tillId === '' ? '' : String(tillId);
  if (wanted) {
    const hit = list.find((row) => rowTillId(row) === wanted && !isUsdCashLog(row));
    if (hit) return hit;
    const balanced = list.filter(hasCashBalanceFields);
    if (balanced.length === 1 && !isUsdCashLog(balanced[0])) return balanced[0];
  }
  const namedTill1 = list.find(
    (row) => isCadTill1Log(row) && /till\s*1\b/i.test(logTillName(row)),
  );
  if (namedTill1) return namedTill1;
  const cadWithTill = list.find((row) => isCadTill1Log(row) && rowTillId(row));
  if (cadWithTill) return cadWithTill;
  const location = list.find((row) => !rowTillId(row)) || null;
  const usd = firstMoney(list, ['start_balance_usd']);
  if (location && !isUsdCashLog(location) && (usd == null || Math.abs(usd) < 0.005)) {
    return location;
  }
  return null;
}
...
/**
 * CAD: Till 1 start_balance / physical only.
 * USD: start_balance_usd / physical_usd only. Never convert. Never add.
 */
function parseCashLogs(payload, dateKey, { tillId } = {}) {
  const rows = cashLogRows(payload);
  const cadRow = pickCadBalanceRow(rows, tillId);
  const usdStart = firstMoney(rows, ['start_balance_usd']) ?? 0;
  const usdPhysical = firstMoney(rows, ['physical_usd']) ?? 0;

  return {
    date: dateKey,
    startBalance:
      readMoney(cadRow, ['start_balance', 'startBalance', 'opening_balance', 'opening']) ?? 0,
    startBalanceUsd: usdStart,
    physical: readMoney(cadRow, ['physical', 'physical_count']) ?? 0,
    physicalUsd: usdPhysical,
    hasUsdBalanceFields: rows.some((row) =>
      rowHasMoneyKeys(row, ['start_balance_usd', 'physical_usd']),
    ),
    raw: payload,
  };
}
...
function isUsdCashRow(row) {
  ...
  if (tillCurrency === 'USD') return true;
  if (isUsdName(row?.tillName)) return true;
  // Invoice currency is often USD for bullion; only till cash_transactions
  // use the row currency as the drawer.
  if (row?.kind === 'cash_transaction') {
...
export async function resolveStoreLocation(session, storeName) {
  ...
  hits.sort((a, b) => {
    const rank = storeSystemRank(a.system.key, target) - storeSystemRank(b.system.key, target);
...
export async function fetchCashLogs(system, { locationId, date, tillId } = {}) {
  ...
  if (firstOk) return parseCashLogs(first.payload, day, { tillId });
  ...
  if (second.ok && cashLogRows(second.payload).some(looksLikeCashLog)) {
    return parseCashLogs(second.payload, day, { tillId });
  }
...
export async function fetchStoreCashPosition(session, { storeName, date } = {}) {
  ...
  const usdPayments = paymentRows.filter((row) =>
    isUsdCashRow(row) || (tills.usd ? rowMatchesTill(row, tills.usd) : false),
  );
  const usdIds = new Set(usdPayments.map((row) => row.id));
  const cadPayments = paymentRows.filter((row) => {
    if (usdIds.has(row.id) || isUsdCashRow(row)) return false;
    if (tills.cad) return rowMatchesTill(row, tills.cad) || (!row.tillId && isTill1CadRow(row));
    return isTill1CadRow(row);
  });
  const usdTxns = cashTransactions.filter((row) =>
    isUsdCashRow(row) || (tills.usd ? rowMatchesTill(row, tills.usd) : false),
  );
  const usdTxnIds = new Set(usdTxns.map((row) => row.id));
  const cadTxns = cashTransactions.filter((row) => {
    if (usdTxnIds.has(row.id) || isUsdCashRow(row)) return false;
    if (tills.cad) return rowMatchesTill(row, tills.cad) || (!row.tillId && isTill1CadRow(row));
    return isTill1CadRow(row);
  });
  const cadIds = new Set(cadPayments.map((row) => row.id).concat(cadTxns.map((row) => row.id)));
  const allUsdIds = new Set([...usdIds, ...usdTxnIds]);
  const otherPayments = paymentRows.filter((row) => !cadIds.has(row.id) && !allUsdIds.has(row.id));
  const otherTxns = cashTransactions.filter((row) => !cadIds.has(row.id) && !allUsdIds.has(row.id));
  ...
  const usd = buildCurrencyDrawer({
    currency: 'USD',
    label: 'USD',
    yesterdayPhysical: yesterdayUsd.physicalUsd || yesterdayCad.physicalUsd,
    todayStart: todayUsd.startBalanceUsd || todayCad.startBalanceUsd,
    todayPhysical: todayUsd.physicalUsd || todayCad.physicalUsd,
  ...
  return {
    ...
    yesterday: {
      ...
      physicalUsd: yesterdayUsd.physicalUsd || yesterdayCad.physicalUsd,
    },
    todayLog: todayCad,
