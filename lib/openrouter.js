...
/**
 * Top band of a page photo, full width. The PO number sits under the date
 * in this strip, so the digits stay large enough to read.
 */
export async function cropTopImageDataUrl(
  dataUrl,
  { fraction = 0.45, maxWidth = 2000, quality = 0.92 } = {},
) {
  if (!dataUrl || typeof document === 'undefined') return dataUrl;
  const band = Math.min(0.7, Math.max(0.28, fraction));

  return new Promise((resolve) => {
    const image = new Image();
    image.onload = () => {
      const cropW = image.width;
      const cropH = Math.max(1, Math.round(image.height * band));
      ...
      ctx.drawImage(image, 0, 0, cropW, cropH, 0, 0, width, height);
      try {
        resolve(canvas.toDataURL('image/jpeg', quality));
      } catch {
        resolve(dataUrl);
