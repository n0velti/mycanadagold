...
import { loadNightCounts } from './bullionNight';
...
/** Physical count that should match system qty: vault + night. */
export function countedPhysicalTotal(vaultCount, nightCount) {
  return (toNumber(vaultCount) ?? 0) + (toNumber(nightCount) ?? 0);
}
...
export async function fetchBullionAudit(session, { date, locationId, systemKey, storeName }) {
  ...
  const [inventory, logs, nightByProduct] = await Promise.all([
    ...
    storeName ? loadNightCounts(storeName) : Promise.resolve({}),
  ]);
  ...
  const rows = products
    .map((product) => {
      ...
      const nightCount = nightByProduct[id] ?? null;
      const history = {};
