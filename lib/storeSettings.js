...
function hoursForDate(settings, date) {
  const key = dateKeyLocal(date);
  const holiday = normalizeHolidays(settings?.holidays).find((row) => row.date === key);
  if (holiday) {
    return {
  ...
  const weekly = normalizeWeeklyHours(settings?.hours);
  return weekly[date.getDay()] || { closed: true, open: '10:00', close: '18:00' };
}
...
export function isStoreOpenNow(settings, at = new Date()) {
  ...
  if (isWithinHours(hoursForDate(settings, now), nowMin)) return true;
  const yesterday = new Date(now.getTime());
  yesterday.setDate(yesterday.getDate() - 1);
  return isWithinHours(hoursForDate(settings, yesterday), nowMin, { overnightOnly: true });
}
